--[[
    ...
    Converted for VerliHub - LUA 5.0 / 5.1 by red_neon (dchub://tmr.dcpp.ru:411/)
...
path = "/etc/verlihub/scripts"


Settings = {
	-- Default Hub BotName "" or "custombot"
	sBot = "Request-Bot",    -- set "" for Main Hub BotName
	sBotTag = "<RRB V:3a>",
	sBotSpeed = "Bot",
	sBotMail = "",
	...
	bTimer = true,
	-- true = Send iMax Requests/Releases on connect; false = Don't
	...
	clrVotesCmd = "clrvote",  TopPosterCmd = "topposter", helpCmd = "shelp", rDoneCmd = "done",
	SetupCmd = "setup", addCatCmd = "addcat", delCatCmd = "delcat", showCatCmd = "showcat",
}

tConfig = {}; tFilled = {}; tOffline = {}; tVote = {}; tabTimers = {n=0}; TmrFreq = 60;
...
function Main()
	-- Register sBot
	if Settings.bReg then
		local _,MainHubBotName = VH:GetConfig("config", "hub_security")
				if (Settings.sBot == "") or (Settings.sBot == nil) then
						sBot = MainHubBotName
				else
                        sBot = Settings.sBot
						VH:AddRobot(sBot,10,Settings.sBotTag,Settings.sBotSpeed.." ",Settings.sBotMail,"0")
				end
	end
	-- Register Timers
	tFunctions.RegTimer(tFunctions.Cleaner, Settings.iCleanDelay*60*60);
	tFunctions.RegTimer(tFunctions.TimedCat, 60);
...
function UnLoad()
   if not ((Settings.sBot == "") or (Settings.sBot == nil)) then
      VH:DelRobot(sBot)
   end
end

function VH_OnUserCommand(user,data)
	-- Parse Main Chat commands
	local s,e,cmd = string.find(data,"+(.+)")
        if s then return tFunctions.ParseCommands(user,cmd) end
return 1
end
	
function VH_OnParsedMsgPM(user, msg, to)
    -- Parse commands
		local s,e,cmd = string.find(msg,"^+(.+)")
		if (to == sBot) and s then return tFunctions.ParseCommands(user,cmd) end
return 1
end
...
tCmds = {
	[Settings.clrVotesCmd] = {
		tFunc = function(user,data)
			...
			SendDataToUserMain(user,"*** All votes have been successfully cleared.")
		end,
		tLevels = {
			[3] = 1,
			[4] = 1,
			...
			if cat and i then
				...
				if tTable[Cat] then
					...
					if tTable[Cat][tonumber(i)] then
						...
						if tVote[Cat][IP] then
							SendDataToUserMain(user,"*** Error: You have already voted.")
						else
							...
							SendDataToUserMain(user,"*** You have successfully voted on \""..tTable[Cat][tonumber(i)].sRel.."\" [Category: "..cat.."].")
							SendDataToOpsPM("*** "..user.." voted on \""..tTable[Cat][tonumber(i)].sRel.."\" [Category: "..cat.."].")
						end
					else
						SendDataToUserMain(user,"*** Error: There is no ID: "..i.." in "..cat)
					end
				else
					SendDataToUserMain(user,"*** Error: There is no Category \""..cat.."\"!")
				end
			else
				SendDataToUserMain(user,"*** Syntax Error: Type +"..tPrefix..Settings.voteCmd.." <category> <ID>")
			end
		end, 
		tLevels = {
			[3] = 1,
	...
	[Settings.TopVotesCmd] = {
		tFunc = function(user)
			...
			SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",9).."Top "..Settings.vMax.." Votes"..
	...
	[Settings.TopPosterCmd]	=	{
		tFunc = function(user)
			...
			SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",8).."Top "..Settings.pMax..
	...
	[Settings.addCatCmd] = {
		tFunc = function(user,data)
			...
			if cat and date then
				...
				-- DB contains it
				if tTable[Cat] then
					SendDataToUserMain(user,"*** Error: There is already a Category: \""..cat.."\"")
				else
					-- Check category´s size
					if (string.len(cat) > Settings.iCatSize) then
						SendDataToUserMain(user,"*** Error: The Category can't have more than "..Settings.iCatSize.." characters.")
					else
						-- Create and save category
						tTable[Cat] = {}; tTable[Cat].iClean = tonumber(date); tFunctions:save(tTable)
						SendDataToUserMain(user,"*** \""..cat.."\" was successfully added to "..Settings.sMode.."s´ categories.")
					end
				end
			else
				SendDataToUserMain(user,"*** Syntax Error: Type +"..tPrefix..Settings.addCatCmd.." "..Settings.sSep.."category"..Settings.sSep.." <maximum time in days>")
			end
		end, 
		tLevels = {
			[3] = 1,
		...
		tFunc = function(user,data)
			...
			if cat then
				...
				if tTable[Cat] then
					...
					SendDataToUserMain(user,"*** \""..cat.."\" was sucessfully deleted from "..Settings.sMode.."s´ categories.")
				else
					SendDataToUserMain(user,"*** Error: There is no Category: "..cat.." in "..Settings.sMode..".")
				end
			else
				SendDataToUserMain(user,"*** Syntax Error: Type +"..tPrefix..Settings.delCatCmd.." <category>")
			end
		end, 
		tLevels = {
			[3] = 1,
	...
	[Settings.showCatCmd] =	{
		tFunc = function(user)
			...
			if next(tTable) then
				...
				SendDataToUserMain(user,msg)
			else
				SendDataToUserMain(user,"*** Error: There are no "..Settings.sMode.." categories!");
			end
		end,
		tLevels = {
			[3] = 1,
	...
	[Settings.addCmd] = {	
		tFunc = function(user,data)
			...
			local s,e,tmp,link = string.find(data,"^%S+%s+"..Settings.sSep.."%S+"..Settings.sSep.."%s+"..Settings.sSep.."(.+)"..Settings.sSep.."%s+(.+)%s*")
			link = link or ""
				...
				if cat then
					...
					if tTable[Cat] then
						if (string.len(rel) > Settings.iRelSize) then
							SendDataToUserMain(user,"*** Error: The "..Settings.sMode.."s can't have more than "..Settings.iRelSize.." characters.")
						else
							local Exists = nil
							...
							if Exists then
								SendDataToUserMain(user,"*** Error: There's already a "..Settings.sMode.." named: \""..rel.."\".")
							else
								table.insert( tTable[Cat], { sRel = rel, sPoster = user, sLink = link, iTime = os.date(), iVote = 0, } ); tFunctions:save(tTable);
								SendDataToAllMain("*** "..user.." added a new "..Settings.sMode..": "..rel..". For more details type: +"..tPrefix..Settings.showCmd)
							end
						end

					else
						SendDataToUserMain(user,"*** Error: There is no Category: \""..cat.."\" in "..Settings.sMode.."s!")
					end
				else
					SendDataToUserMain(user,"*** Syntax Error: Type +"..tPrefix..Settings.addCmd.." <category> <type>")
				end
			else
				SendDataToUserMain(user,"*** Error: Type +"..tPrefix..Settings.addCmd.." "..Settings.sSep..
		...
		tFunc = function(user,data)
			...
			if cat and rel then
				...
				if tonumber(rel) then
					...
					if Deleted then 
						SendDataToUserMain(user,"*** ID: "..rel.." was successfully deleted."); tFunctions:save(tTable)
					else
						SendDataToUserMain(user,"*** Error: There is no ID: "..rel..".")
					end
				-- DB contains Cat
				elseif tTable[Cat] then
					...
					if Deleted then
						SendDataToUserMain(user,"Category: \""..cat.."\" was succesfully cleaned up."); tFunctions:save(tTable)
					else
						SendDataToUserMain(user,"*** Error: There is no Category: \""..cat.."\"")
					end
				else
					SendDataToUserMain(user,"*** Error: There is no ID/Category: \""..cat.."\"")
				end
			else
				SendDataToUserMain(user,"*** Syntax Error: Type +"..tPrefix..Settings.delCmd.." "..Settings.sSep.."%[line:{}]"..Settings.sSep.." <ID> / Empty>")
			end
		end, 
		tLevels = {
			[3] = 1,
			...
			if cat then
				if cat == "all" then
					...
					if string.lower(Settings.sMode) == "request" and next(tFilled) then
						SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",9).."Showing all "..Settings.sMode.."s ["..
				...
				elseif tTable[string.lower(cat)] then
					SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",8).."Showing all "..Settings.sMode.."s from \""..cat.."\" "..
					string.rep("\t",7).."["..os.date().."]\r\n",tFunctions.Releaser(4,cat),tTable))
				end
			else
				-- Data sent according to Selected mode and if tFilled isn't empty
				if string.lower(Settings.sMode) == "request" and next(tFilled) then
					SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",9).."Last "..Settings.iMax.." "..Settings.sMode.."s "..
					string.rep("\t",8).."["..os.date().."]\r\n",tFunctions.ShowX(tTable),tTable)..tFunctions.Structure(string.rep("\t",9)..
					"Last "..Settings.fMax.." Filled Requests"..string.rep("\t",7).."["..os.date().."]\r\n",tFunctions.ShowX(tFilled),tFilled))
				else
					SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",8).."Last "..Settings.iMax.." "..Settings.sMode.."s per Category "..
	...
	[Settings.findCmd] = { 
		tFunc = function(user,data)
			...
			if str then
				SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",9).."Search Results of: \""..str.."\""..string.rep("\t",7)..
				"["..os.date().."]\r\n",tFunctions.Releaser(2,str),tTable))
			else
				SendDataToUserMain(user,"*** Error: Type +"..tPrefix..Settings.findCmd.." "..Settings.sSep.."string"..Settings.sSep)
			end
		end,
		tLevels = {
			[0] = 1,
	...
	[Settings.delAllCmd] = { 
		tFunc = function(user)
			...
			SendDataToUserMain(user,"All "..Settings.sMode.."s have been deleted successfully!"); tFunctions:save(tTable)
		end,
		tLevels = {
			[3] = 1,
			[4] = 1,
	...
	[Settings.helpCmd] = { 
		tFunc = function(user)
		local res,Class = VH:GetUserClass(user)
			...
			SendDataToUserMain(user, sMsg.."\t"..string.rep("-",220));
		end,
		tLevels = {
			[0] = 1,
			[1] = 1,
			...
			if string.lower(Settings.sMode) == "request" then
				if cat and rel then
					...
					if tTable[Cat] and tTable[Cat][rel] then
						...
						table.insert(tFilled[Cat], { sPoster = user, sRel = tmp.sRel, iVote = tmp.iVote, iTime = os.date(), sLink = tmp.sLink } )
					end
					if Exists then 
						-- Send/Store tFilled Report
						local tmp = tTable[Cat][rel]
						local msg, nick = "*** Your request \""..tmp.sRel.."\" has been filled by "..user..".", tmp.sPoster;
						if (IsUserOnline(nick) == 1) then SendDataToUserPM(nick,msg) else tOffline[tmp.sPoster] = msg end; table.remove(tTable[Cat],rel);
						SendDataToAllMain("*** "..user.." filled up Request \""..tmp.sRel.."\" [ "..cat.." ]!")
					...
					SendDataToUserMain(user,"*** Error: Type +"..tPrefix..Settings.rDoneCmd.." "..Settings.sSep.."Category"..Settings.sSep.." <ID>")
				end
			else
				SendDataToUserMain(user,"*** Error: This command is only available in Request Mode!")
			end
		end,
		tLevels = {
			[0] = 1,
	...
	[Settings.SetupCmd] = { 
		tFunc = function(user,data)
			...
			if set and value then
				local tTable = {
					["cleaner"] = {
						["on"] = { tFunc = function() tConfig.Cleaner = "on" end, sMode = "enabled" },
						["off"] = { tFunc = function() tConfig.Cleaner = "off" end, sMode = "disabled" },
				...
				if tTable[string.lower(set)][string.lower(value)] then
					...
					SendDataToUserMain(user,tmp.sMsg..tmp[string.lower(value)].sMode.."!")
				else
					SendDataToUserMain(user,"*** Error: Type +"..tPrefix..Settings.SetupCmd.." <link/cleaner> <on/off>")
				end
			else
				SendDataToUserMain(user,"*** Error: Type +"..tPrefix..Settings.SetupCmd.." <link/cleaner> <on/off>")
			end
...
function VH_OnTimer()
if Settings.bTimer then
	if tConfig.Cleaner == "on" then
		if os.time() > TimerNext then
				TimerNext = os.time() + TmrFreq
		end
		if os.time() == TimerNext then
				TimerNext = os.time() + TmrFreq
...
function VH_OnUserLogin(user)
	...
	if Settings.bSendOnConnect then
		SendDataToUserPM(user,tFunctions.Structure(string.rep("\t",9).."Last "..Settings.iMax.." "..Settings.sMode.."s per Category "..
	...
	if Settings.bSendRC then
			...
			for i in ipairs(tRC) do 
				VH:SendDataToUser("$UserCommand 1 3 "..Settings.sMode.." Bot\\"..tRC[i].."&#124;|",user)
			end;
			collectgarbage(); io.flush();
	end
	-- Offline tFilled Report
	if tOffline[user] then SendDataToUserPM(user,tOffline[user]); tOffline[user] = nil end
return 1
end
...
tFunctions.TimedCat = function()
	if Settings.bTimedCat and Settings.Times[os.date("%H:%M")] then
		...
		SendDataToAllMain(TimedMain(Settings.Times[os.date("%H:%M")]))
	end
	collectgarbage(); io.flush();
end
...
tFunctions.Cleaner = function()
	if (tConfig.Cleaner == "on") then -- RegCleaner based
		...
		if clnd ~= 0 and Settings.bCleanReport then 
			SendDataToAllMain("*** "..chkd.." "..Settings.sMode.."(s) were processed. "..clnd.." "..Settings.sMode..
...
tFunctions.ParseCommands = function(user,data)
	...
	if cmd and tCmds[string.lower(cmd)] then
		...
		if tCmds[cmd].tLevels[Class] then
			return tCmds[cmd].tFunc(user,data), 1
		else
			return SendDataToUserMain(user,"*** Error: You are not allowed to use this command."), 1
		end
	else
	return 1
	end
...
function IsUserOnline(nick)
	local res, nicklist = VH:GetNickList()
	local res = 1

		if res then
			local nicklist = string.sub(nicklist, 11)
			for user in string.gfind(nicklist, "([^$]+)%$%$") do
				if string.lower(user) == string.lower(nick) then
				return 1
				end
			end
		end
...
--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
function SendDataToUserMain(nick, data)
	return VH:SendDataToUser("<"..sBot.."> "..data.."|",nick)
end


function SendDataToUserPM(nick, data)
	return VH:SendDataToUser("$To: "..nick.." From: "..sBot.." $<"..sBot.."> "..data.."|",nick)
end


function SendDataToAllMain(data)
	return VH:SendDataToAll("<"..sBot.."> "..data.."|",0, 10)
end


function SendDataToAllPM(data)
	return VH:SendPMToAll(data.."|",sBot ,0, 10)
end


function SendDataToOpsMain(data)
	return VH:SendDataToAll("<"..sBot.."> "..data.."|",3, 10)
end


function SendDataToOpsPM(data)
	return VH:SendPMToAll(data.."|",sBot ,3, 10)
end
--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::